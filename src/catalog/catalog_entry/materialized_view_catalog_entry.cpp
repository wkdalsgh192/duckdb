...
void MaterializedViewCatalogEntry::Initialize(CreateMaterializedViewInfo &info) {
...
MaterializedViewCatalogEntry::MaterializedViewCatalogEntry(Catalog &catalog, SchemaCatalogEntry &schema,
    ...
    : StandardEntry(CatalogType::MATERIALIZED_VIEW_ENTRY, schema, catalog, info.view_name) {
	Initialize(info);
}

unique_ptr<CreateInfo> MaterializedViewCatalogEntry::GetInfo() const {
	auto result = make_uniq<CreateMaterializedViewInfo>();
...
unique_ptr<CatalogEntry> MaterializedViewCatalogEntry::AlterEntry(ClientContext &context, AlterInfo &info) {
	D_ASSERT(!internal);
	...
	if (info.type == AlterType::SET_COLUMN_COMMENT) {
		...
		for (idx_t i = 0; i < names.size(); i++) {
			...
			if (col_name == comment_on_column_info.column_name) {
				auto &copied_view_entry = copied_view->Cast<MaterializedViewCatalogEntry>();
...
string MaterializedViewCatalogEntry::ToSQL() const {
	if (sql.empty()) {
...
unique_ptr<CatalogEntry> MaterializedViewCatalogEntry::Copy(ClientContext &context) const {
	D_ASSERT(!internal);
	auto create_info = GetInfo();

	return make_uniq<MaterializedViewCatalogEntry>(catalog, schema, create_info->Cast<CreateMaterializedViewInfo>());
}
